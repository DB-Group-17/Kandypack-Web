...
- [ ] Full baseline seed data (`seed_data_spec.md`, all sections) loaded into the shared dev DB — ⏳ **partially done.** §1–§9 and §12 are loaded. §10 (truck schedules, deliveries) and §11 (store inventory, inventory transactions) are still outstanding — as of 2026-09-26 the shared dev DB has 0 rows in `truck_schedules`, `deliveries`, `inventory_transactions` and `store_inventory`. No longer blocked by Member 3 (truck scheduling is merged); still needs the open question in `06_seed-data-spec.md` §9 settled first: historical orders carry no train bookings, so completed deliveries have no received stock to dispatch against.
