...
-- Fix: place_order failed for every caller with
--      "Illegal mix of collations ... for operation '='"
--
-- Cause: route_coverage_areas.area_name is declared
--        COLLATE utf8mb4_general_ci (04_routes.sql), while the
--        p_delivery_area parameter takes the database default
--        (utf8mb4_0900_ai_ci). The coverage-area lookup compared
--        the two directly, so the procedure aborted before any
--        business logic ran — POST /api/orders could never create
--        an order.
--
-- Fix: force the comparison into the column's own collation.
--      Matching stays case-insensitive, as the procedure's
--      contract and Docs/06_seed-data-spec.md §5 both assume.
--
-- This is the ONLY change from 17_proc_place_order.sql; the whole
-- body is carried forward verbatim because procedures are replaced
-- wholesale, not altered in place. Migration 17 cannot be edited
-- instead: it is already recorded in _schema_migrations on every
-- existing database, so the runner would never re-apply it.
-- =========================================================
...
BEGIN
    ...
    -- COLLATE utf8mb4_general_ci: the only change from migration 17 (see header).
