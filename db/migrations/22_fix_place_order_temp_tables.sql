...
-- Two further defects in place_order, both of which meant the
-- procedure could never complete on this database.
--
-- 1. tmp_trip_alloc was created without a primary key. Aiven runs
--    MySQL 8.4 with sql_require_primary_key = ON, so the CREATE
--    TEMPORARY TABLE failed with ER_TABLE_WITHOUT_PK and the
--    procedure aborted before booking any space. order_item_id is
--    unique within one trip's allocation, so it is the natural key.
--
-- 2. TRUNCATE TABLE is DDL and forces an implicit COMMIT. Every
--    call therefore committed the order and its items mid-procedure,
--    so a later failure (no trip with capacity, a rejected capacity
--    check) left a committed order with no bookings, and a caller's
--    own transaction could not roll the order back. This contradicted
--    Docs/05_api-and-pages.md, which states that any failure inside
--    the procedure rolls the whole transaction back. DELETE is DML
--    and commits nothing, so atomicity is restored.
--
-- Everything else is carried forward verbatim from migration 21
-- (which fixed the coverage-area collation mismatch). Migrations 17
-- and 21 cannot be edited instead: both are already recorded in
-- _schema_migrations, so the runner would never re-apply them.
-- =========================================================
...
BEGIN
    ...
    -- COLLATE utf8mb4_general_ci: carried forward from migration 21 (see header).
    SELECT route_id INTO v_route_id FROM route_coverage_areas
     WHERE city_id=p_destination_city_id
       AND area_name=p_delivery_area COLLATE utf8mb4_general_ci LIMIT 1;
    ...
    DELETE FROM tmp_item_remaining;
    ...
    CREATE TEMPORARY TABLE IF NOT EXISTS tmp_trip_alloc(order_item_id BIGINT PRIMARY KEY, qty DECIMAL(10,2), space DECIMAL(10,2));
